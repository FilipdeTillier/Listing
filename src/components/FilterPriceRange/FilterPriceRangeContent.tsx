...
type FilterPriceRangeContentProps = {
  open: boolean;
  close: () => void;
  max: number | null;
  min: number | null;
...
export const FilterPriceRangeContent: React.FC<
  FilterPriceRangeContentProps
> = ({ open, close, min, max, onReset, onChange }) => {
  ...
  useEffect(() => {
    setValues([min ?? MIN_SALARY, max ?? DEFAULT_MIN_SALARY]);
  }, [open]);
                ...
                <div>
                  ...
                  <div className="mt-1 relative rounded-md">
                    ...
                    <input
                      ...
                      id="maxPrice"
                      className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-3 sm:text-sm border-neutral-200 rounded-full text-neutral-900 "
                      value={values[1] || DEFAULT_MIN_SALARY}
                    />
                  </div>
                </div>
              </div>
